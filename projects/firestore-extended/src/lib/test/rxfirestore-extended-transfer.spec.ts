...
describe('RxFire Firestore Extended transferItemInIndexedDocs', () => {
  ...
  beforeEach((done: DoneFn) => {
    ...
    testCollectionRef = firestore.collection(`${collectionName}_${createId()}`) as CollectionReference<DragAndDropItem>;
  ...
  it('useCopy = false, update modifiedDate', (done: DoneFn) => {
    ...
    let collectionA = firestore
      .collection(testCollectionRef.path)
      .where('groupName', '==', 'A')
      .orderBy('index', 'desc') as CollectionReference<DishItem>;

    let collectionB = firestore
      .collection(testCollectionRef.path)
      .where('groupName', '==', 'B')
      .orderBy('index', 'desc') as CollectionReference<DishItem>;
